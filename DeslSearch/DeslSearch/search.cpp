...
//固定了活跃S盒个数，固定了活跃S盒模式，固定了概率模式并得到了概率和，固定了活跃输入差分，遍历输出差分并进入下一轮。运用了性质：第二轮遍历可以固定输入差分的第一个S盒始终活跃。
//N1 活跃S盒个数
//C0[0]~C0[N1-1] 活跃S盒模式
//a2[0]~a2[N1-1] 概率模式
...
//输入差分保存在r_od_l当中了
//r 当前轮数，判断是否进入最后一轮用
//第一个S盒不一定活跃，不必遍历7个等价的输入输出差分。
//并且在这里输入差分在相应概率上没有输出差分，所以m[i]有零则退出。
...
void Round_j(int r){
	...
	tmp0=_mm_xor_si128(*dp1,*dp2);
	_mm_store_si128(idp,tmp0);//获得输入差分
	getInfo(r-1,*idp);//获得S盒个数与位置
